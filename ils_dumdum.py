                    ...
                    if 'value' in graph_data2:
                        for i in graph_data2['value']:
                            ...
                            try:
                                ...
                                b = datetime.strptime(i['receivedDateTime'], format).replace(
                                ...
                                with mysql.connector.connect(**conn_data) as con:
                                    cur = con.cursor()
                                    cur.execute(f"select * from {table_name} where subject=%s and date=%s limit 1", (subject, date))
                                    temp_var = cur.fetchone()
                                    if temp_var is not None:
                                        print(datetime.now(), ' got mail in db')
                                        exist = True
                                if exist is False:
